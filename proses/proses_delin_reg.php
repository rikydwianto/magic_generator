...
$by_staff = array();
?>
<h1>SEDANG PROSES DELIN REGIONAL <?= $nama_cabang ?></h1>
<h2>Harap tunggu . . .</h2>
<?php
...
$sheet1->getStyle('A2:P2')->getFont()->setBold(true);




$headerData = [
    'NO', 'CABANG', 'LOAN', 'CENTER', 'ID AGT', 'ANGGOTA',  'TANGGAL DISBURSE', 'DISBURSE',
...
$headerData = [
    'NO', 'CABANG', 'LOAN', 'CENTER', 'ID AGT', 'ANGGOTA', 'TGL DISBURSE', 'DISBURSE',
...
$judul = "REKAP SEMUA PAR PER STAFF dari $tgl_delin_awal s/d $tgl_delin_akhir \n$nama_cabang";
$sheet4->setCellValue('A1', $judul);
$sheet4->setAutoFilter('A2:L2');
...
$sheet5->setAutoFilter('A2:T2');
$headerData = array(
    'NO', 'CABANG', 'LOAN', 'CTR', 'CLIENT ID', 'NASABAH', 'PRODUK',
...
if ($hitung_naik > 0) {
    foreach ($stmt->fetchAll() as $row) {
        ...
        $isiData = [
            $no, $row['loan'], $row['cabang'], $row['no_center'], $row['id_detail_nasabah'], $row['nasabah'],  $row['tgl_disburse'], $row['amount'],
            $row['sisa_saldo'], $row['tunggakan'], $row['minggu'], $row['minggu_rill'], $row['priode'], $row['jenis_topup'], $row['hari'], $row['staff']
        ];

        $sql_update = "update deliquency_regional set keterangan='naik' where id='$row[id]'";
    ...
    $sheet1->mergeCells('A' . $baris_1 . ':G' . $baris_1);
...
$sheet1->mergeCells('A' . $baris_baru . ':P' . $baris_baru);
...
$barisPertama->setRowHeight(40);
$mergedCell->getStyle()->getAlignment()->setWrapText(true);
$sheet1->getStyle('A' . $header_baru . ':P' . $header_baru)->getFont()->setBold(true);


$baris_turun = $header_baru + 1;
$sql_turun = "SELECT * FROM deliquency_regional WHERE `loan` NOT IN (SELECT loan FROM deliquency_regional WHERE tgl_input = '$tgl_delin_akhir') AND tgl_input = '$tgl_delin_awal'AND regional = '$nama_cabang' order by cabang,staff asc,hari desc";
...
if ($hitung_turun > 0) {
    foreach ($stmt->fetchAll() as $row) {
        ...
        $sql_update = "update deliquency_regional set keterangan='turun' where id='$row[id]'";
        $upd = $pdo->query($sql_update);
        // Menuliskan isi ke dalam sheet
        $column = 'A';
        foreach ($isiData as $isi) {
    ...
    $sheet1->mergeCells('A' . $baris_turun . ':G' . $baris_turun);
...
$sql_kurang = "SELECT * FROM deliquency_regional WHERE `loan` IN (SELECT loan FROM deliquency_regional WHERE tgl_input = '$tgl_delin_awal') AND tgl_input = '$tgl_delin_akhir'AND regional = '$nama_cabang' order by cabang,staff asc,hari desc";
...
foreach ($stmt->fetchAll() as $row) {
    $sq_saldo = "select sisa_saldo from deliquency_regional where loan='$row[loan]' and tgl_input='$tgl_delin_awal' and regional='$nama_cabang'";
    ...
    if ($saldo_berkurang > 0) {
        $isidata = [
            $no, $row['cabang'], $row['loan'], $row['no_center'], $row['id_detail_nasabah'], $row['nasabah'], $row['tgl_disburse'], $row['amount'],
        ...
        // Menuliskan header ke dalam sheet pada baris ke-2
        $sql_update = "update deliquency_regional set keterangan='turunos',perubahan='$saldo_berkurang' where id='$row[id]'";
        $upd = $pdo->query($sql_update);
        $column = 'A';
        foreach ($isidata as $isi) {
...
$no = 1;
$sql_rekap = "SELECT cabang,staff, COUNT(DISTINCT no_center) AS ctr_par,SUM(sisa_saldo) AS saldo,COUNT(DISTINCT id_detail_nasabah) AS agt_par,COUNT(id_detail_nasabah) AS rek_par FROM deliquency_regional WHERE tgl_input='$tgl_delin_akhir' AND regional='$nama_cabang' GROUP BY staff order by cabang,staff asc";
...
$sheet4->mergeCells('A' . $baris_4 . ':C' . $baris_4);
...
$no = 1;
$sql_tdk_bayar = "SELECT * FROM deliquency_regional WHERE `loan` IN (SELECT loan FROM deliquency_regional WHERE tgl_input = '$tgl_delin_akhir') AND tgl_input = '$tgl_delin_awal' AND regional = '$nama_cabang' order by cabang,staff asc,hari desc";
$stmt = $pdo->query($sql_tdk_bayar);
$baris_5 = 3;
foreach ($stmt->fetchAll() as $row) {
    $sql_banding = "SELECT * FROM deliquency_regional WHERE loan ='$row[loan]' AND tgl_input = '$tgl_delin_akhir' AND regional = '$nama_cabang' ";
    ...
    if ($saldo_after == $saldo_before) {
        ...
        $headerData = array(
            $no,  $row['cabang'], $row['loan'],  $row['no_center'],  $row['id_detail_nasabah'], $row['nasabah'], $row['kode_pemb'],
...
$sheet1->getStyle('A2:P' . $batas_sh1)->applyFromArray($styleArray); //INI UNTUK BORDER
foreach (range('A', 'P') as $col) {
...
$sheet1->getStyle('A' . $batas_sh2 . ':P' . $akhir_batas_sh2)->applyFromArray($styleArray); //INI UNTUK BORDER

//SHEET 3/ pengurangan oS par

$batas_sh3 = $baris_3;
$sheet3->getStyle('G3:K' . $batas_sh3)->getNumberFormat()->setFormatCode('#,##0');
$sheet3->getStyle("A2:Z" . $batas_sh3)->getFont()->setSize(8);
$sheet3->getStyle('A2:O' . $batas_sh3)->applyFromArray($styleArray); //INI UNTUK BORDER
...
$batas_sh4 = $baris_4;
$sheet4->getStyle('D3:L' . $batas_sh4)->getNumberFormat()->setFormatCode('#,##0');
$sheet4->getStyle("A2:Z" . $batas_sh4)->getFont()->setSize(8);
$sheet4->getStyle('A2:L' . $batas_sh4)->applyFromArray($styleArray); //INI UNTUK BORDER
...
$batas_sh5 = $baris_5;
$sheet5->getStyle('D3:T' . $batas_sh5)->getNumberFormat()->setFormatCode('#,##0');
$sheet5->getStyle("A2:AA" . $batas_sh5)->getFont()->setSize(8);
