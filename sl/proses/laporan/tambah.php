        ...
        <div class="row">
            <div class="col-lg-6 ">
                <div class="mb-3">
                    ...
                    <input type="text" readonly class="form-control" value='<?= $detailAkun['nik_staff'] ?>'
                        name="nik_staff" required>
                </div>

                <div class="mb-3">
                    <label for="nama_staff" class="form-label">Nama Staff:</label>
                    <input type="text" readonly class="form-control" value='<?= $detailAkun['nama_staff'] ?>'
                        name="nama_staff" required>
                </div>

                <div class="mb-3">
                    <label for="cabang_staff" class="form-label">Cabang Staff:</label>
                    <input type="text" readonly class="form-control" value='<?= $detailAkun['cabang'] ?>'
                        name="cabang_staff" required>
                </div>
            ...
            <div class="col-lg-6 ">
                <div class="row g-3">
                    <div class="col">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" readonly class="form-control" value='<?= $detailAkun['regional'] ?>'
                    ...
                    <div class="col">
                        ...
                        <input type="number" readonly class="form-control" value='<?= $detailAkun['wilayah'] ?>'
                            name="wilayah" required>
                    </div>
                </div>
                ...
                <button type="submit" name='tambah' class="btn btn-primary mt-3">Lanjut <i
...
if (isset($_POST['tambah'])) {
    try {
        ...
        $checkQuery = "SELECT COUNT(*) FROM capaian_cabang 
        WHERE nama_cabang = :cabang AND minggu = :minggu AND bulan =:bulan and tahun = :tahun and status='done'";

        // Persiapkan statement
        $checkStmt = $pdo->prepare($checkQuery);

        // Bind parameter
        $checkStmt->bindParam(':cabang', $cabang_staff);
        $checkStmt->bindParam(':minggu', $minggu);
        $checkStmt->bindParam(':bulan', $bulan);
        $checkStmt->bindParam(':tahun', $tahun);
        ...
        if ($rowCount > 0) {
            alert("Sudah tidak dapat laporan, karena laporan sudah di submit ke regional!");
            pindah(menu_sl("laporan/tambah"));
        }
